...
// STEP 1: Retrieve data from API and presented on the console
// OPERATOR: fromFetch https://rxjs.dev/api/fetch/fromFetch
// OPERATOR: switchMap https://rxjs.dev/api/operators/switchMap
const giphyApi$ = fromFetch(`${giphyApiURL}/${giphySearchTag}?q=${search_term}&api_key=${giphyApiKey}&limit=${search_limit}`).pipe(
  switchMap(response => {
    if (response.ok) {
      // OK return data
